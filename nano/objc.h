...
namespace cf {
  ...
  inline unique_ptr<CFDictionaryRef> create_dictionary(CFStringRef const (&keys)[N], CFTypeRef const (&values)[N]);
} // namespace cf.
...
namespace cf {
  ...
  template <class _CFType>
  struct unique_ptr : std::unique_ptr<std::remove_pointer_t<_CFType>, object_deleter> {
    using base = std::unique_ptr<std::remove_pointer_t<_CFType>, object_deleter>;

    inline unique_ptr(_CFType ptr)
        : base(ptr) {}
...
namespace objc {
  ...
  obj_t* create_object(const char* classType, const char* initFct, Params&&... params) {
    ...
    objc::call<R, Args...>(obj, initFct, std::forward<Params>(params)...);
    return obj;
  }
